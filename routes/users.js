...
router.patch('/setAuth', async function(req, res){
	...
	if(q_res.success){
		...
		if(body.isAuthSuccess){
			...
			if(body.houseType == 1 && addressData.buildingName){//아파트별 토픽 생성
				insertTopic(`building_${addressData.buildingName}`, addressData.buildingName)
				insertTopicUser(`building_${addressData.buildingName}`, body.userId)
			}
		}
		res.status(200).json({data:null})
	}else{
